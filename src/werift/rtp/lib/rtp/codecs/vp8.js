...
class VP8 {
    static payLoader(mtu, payload) {
        ...
        while (payloadDataRemaining > 0) {
            const currentFragmentSize = Math.min(maxFragmentSize, payloadDataRemaining);
            const out = {};
            ...
            payloads.push(Buffer.from(Object.keys(out)
                .sort()
                .map((i) => out[Number(i)])));
