...
class Connection {
    constructor(iceControlling, options) {
        ...
        this.queryConsent = () => new p_cancelable_1.default(async (r, f, onCancel) => {
            ...
            while (!this.remoteIsLite) {
                ...
                await utils_1.sleep(CONSENT_INTERVAL * (0.8 + 0.4 * Math.random()) * 1000);
    ...
    async getComponentCandidates(component, addresses, timeout = 5, cb) {
        ...
        for (const address of addresses) {
            ...
            await protocol.connectionMade(net_1.isIPv4(address));
        ...
        if (this.turnServer &&
            ...
            const protocol = await protocol_1.createTurnEndpoint(this.turnServer, this.options.turnUsername, this.options.turnPassword);
            this.protocols.push(protocol);
